...
$data = ProjectStages::orderBy('order_list')->get();
return view('backend.pages.project_stages.index',compact('data') );
}

public function create()
{
if ( Gate::denies('create_project_stages') ) { abort(403); }
return view('backend.pages.project_stages.create' );
}
...
ProjectStages::create($request->all());
ResponseCache::clear();
Session::flash('msg',  trans('lang.success_created') );
Session::flash('alert', 'success');
return Redirect(config('settings.BackendPath').'/project_stages');
}

public function edit($id)
{
if ( Gate::denies('edit_project_stages') ) { abort(403); }
$data  = ProjectStages::where('id',$id)->first();
return view('backend.pages.project_stages.edit', compact('data')  );
}

public function update(Request $request, $id)
{
if ( Gate::denies('edit_project_stages') ) { abort(403); }
ProjectStages::find($id)->update($request->all());
ResponseCache::clear();
