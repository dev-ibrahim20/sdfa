...
<h4 class="header-title mb-6">{{__('lang.update_project_stages')}} | {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/project_stages/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.project_stages.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_project_stages')}} | {{$data->title}} | {{config('settings.ProjectName')}}</title>
