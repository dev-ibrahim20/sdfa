...
{{-- <div class="alert alert-danger" role="alert">
  Job Order Number #523 Exceeding the Qty limit 
</div> --}}

{{-- <div class="alert alert-warning" role="alert">
Job Order Number #742 late in finishing
</div> --}}
...
<p class="text-muted mb-1 text-truncate"> <a href="{{url('')}}/{{config('settings.BackendPath')}}/samplings">  العينات </a> </p>
</div>
</div>
</div>
...
<p class="text-muted mb-1 text-truncate">  <a href="{{url('')}}/{{config('settings.BackendPath')}}/samplings">   العينات المستلمه   </a></p>
</div>
</div>
</div>
...
<p class="text-muted mb-1 text-truncate"> <a href="{{url('')}}/{{config('settings.BackendPath')}}/samplings?status=rejected">  العينات المرفوضه </a></p>
</div>
</div>
</div>
...
<div class="col-6">
<div class="text-right">
<h3 class="text-dark my-1"><span data-plugin="counterup">{{ \App\Models\Workplace::count() }}</span></h3>
<p class="text-muted mb-1 text-truncate"><a href="{{url('')}}/{{config('settings.BackendPath')}}/workplaces">المنافذ</a></p>
</div>
</div>
</div>
...
<!--
<div class="card-box">
<h4 class="header-title mb-4"> {{__('lang.warehouses')}}  </h4>
...
<div class="card-box">
wwww
<h4 class="header-title mb-4"> {{__('lang.today_job_orders')}} ({{count($job_orders)}}) </h4>

<table class="table table-hover m-0 table-centered dt-responsive nowrap w-100" id="tickets-table">
<thead>
<tr>

<th> {{__('lang.job_order')}} </th>
<th> {{__('lang.project')}} </th>
<th> {{__('lang.building')}} </th>
<th> {{__('lang.stage')}} </th>
</tr>
</thead>
...
<tr>
<td>{{$job_order->id}}#{{$job_order->title}}</td>
<td>{{$job_order->building->project->title}}</td>
<td>{{$job_order->building->title}}</td>
<td>{{$job_order->stage->title}}</td>
</tr>
@endforeach
...
function drawChart() {
    var data = google.visualization.arrayToDataTable([
        ...
        ['تم استلامها', {{$samplingCounts['delivered']}}],
        ['لم يتم استلامها', {{$samplingCounts['not_delivered']}}],
        ['لم يتم سحبها', {{$samplingCounts['not_been_withdrawn']}}],
        ['مرفوضه', {{$samplingCounts['rejected']}}],
        ...
        function drawChart() {
            ...
            data.addRows([
                @foreach($statistics as $index => $statistic)
                    ['{{ $statistic->sector->name }}', {{ $statistic->count }}, '{{ $statistic->sector->name }}: {{ $statistic->count }}', '{{ $colors[$index % count($colors)] }}'],
                @endforeach
            ]);
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        // Add the data from PHP array
        data.addRows([
            @foreach($workplace_id as $index => $workplaces)
                ['{{$workplaces->workplace->name ??'-' }}', {{ $workplaces->count }}, '{{ $colors[$index % count($colors)] }}'],
            @endforeach
        ]);
