...
@can('create_vendors')
<a href="{{url('')}}/{{config('settings.BackendPath')}}/vendors/create"><button type="button" class="btn btn-danger btn-rounded mb-3"><i class="mdi mdi-plus"></i> {{__('lang.create_vendor')}} </button></a>
@endcan
...
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/vendors/{{$vendor->id}}/edit"> <i class="fas fa-pencil-alt"> </i> {{__('lang.edit')}}</a>


<a href="#" class="dropdown-item get_activity_log" subject_id="{{$vendor->id}}" subject_type="{{get_class($vendor)}}" data-toggle="modal" data-target="#activityLogModal"> <i class="fa fa-history"> </i> {{__('lang.activity_list')}} </a>

</div>
</div> <!-- end dropdown -->
<!-- Title-->
<h5 class="mt-0"><a href="javascript: void(0);" class="text-dark">  {{$vendor->title}}</a></h5>
...
<title> {{__('lang.vendors')}} | {{config('settings.ProjectName')}}</title>
