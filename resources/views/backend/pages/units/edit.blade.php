...
<p class="header-title mb-6"> <B> {{__('lang.update_unit')}} | {{$unit->title}} </B> </p>
{!! Form::model( $unit ,[ 'url' =>  config('settings.BackendPath').'/units/'.$unit->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
...
<title>{{__('lang.update_unit')}} | {{$unit->title}} | {{config('settings.ProjectName')}}</title>
