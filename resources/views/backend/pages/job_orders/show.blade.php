...
<p class="header-title mb-6"> <B> {{$job_order->title_ar}} - {{$job_order->building->title ?? ''}} </B> </p>


<div class="row">

<div class="col-md-6">
<li>Serial: {{$job_order->id}}</li>
<li>Job Order: {{$job_order->title_ar}} - {{$job_order->building->title ?? ''}}</li>
<li>Planned Start Date: {{ $job_order->planned_start_date}}</li>
<li>Actual Start Date: {{ $job_order->actual_start_date }}</li>
<li>Planned Finish Date: {{ $job_order->planned_finish_date}}</li>
<li>Actual Finish Date: {{ $job_order->actual_finish_date }}</li>
</div>



<div class="col-md-6">
<li>Location: {{$job_order->building->title ?? ''}} - {{$job_order->floor->title ?? ''}} Floor  - LCR:{{$job_order->lcr->title ?? ''}} - Item:{{$job_order->task->title ?? ''}}</li>
<li>Stage: {{ $job_order->stage->title ?? ''}}</li>
<li>Category: {{ $job_order->category->title ?? ''}}</li>
<li>Contractor: {{ $job_order->contractor->title ?? ''}}</li>
</div>
...
<div class="col-md-12">
{!! $job_order->activity_description!!}
</div>
...
<title>{{__('lang.update')}} | {{$job_order->title}} | {{config('settings.ProjectName')}}</title>
