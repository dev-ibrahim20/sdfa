...
@foreach($job_order->tasks as $k=>$task)
<tr>
<td><a href="{{url('')}}/{{config('settings.BackendPath')}}/tasks/{{$task->id}}">{{$task->title}}</a></td>
<td> <div style="background-color:{{ $task->stage->color ?? ''}}" class="badge bg-soft-success mb-3">
{{ $task->stage->title ?? ''}} </div></td>
...
@if( $task->stage->is_editable )

@can('edit_projects')
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/task/{{$task->id}}/edit"> <i class="fas fa-pencil-alt"> </i> {{__('lang.edit')}}</a>
@endcan
...
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/task_update_stage/{{$task->id}}/{{ $stage->id }}"> <i class="fab fa-wpforms  "> </i>  {{ $stage->title }} </a>

@endif
@endforeach
@endcan

<a href="#" class="dropdown-item get_activity_log" subject_id="{{$task->id}}" subject_type="{{get_class($task)}}" data-toggle="modal" data-target="#activityLogModal"> <i class="fa fa-history"> </i> {{__('lang.activity_list')}} </a> 
</div>
</div>
</td>
