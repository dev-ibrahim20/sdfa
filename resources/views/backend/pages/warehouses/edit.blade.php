...
<p class="header-title mb-6"> <B> {{__('lang.update_warehouse')}} | {{$warehouse->title}} </B> </p>

{!! Form::model( $warehouse ,[ 'route' => ['warehouse_update',$warehouse->id] , 'method'=>'POST' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.warehouses.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_warehouse')}} | {{$warehouse->title}} | {{config('settings.ProjectName')}}</title>
