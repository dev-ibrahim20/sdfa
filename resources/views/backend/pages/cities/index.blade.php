...
@can('create_cities')
<a href="{{url('')}}/{{config('settings.BackendPath')}}/cities/create" class="btn btn-sm btn-blue waves-effect     waves-light float-right">
<i class="mdi mdi-plus-circle"></i> {{__('lang.add_new')}}
</a>
@endcan

<h3 class="header-title mb-4">{{__('lang.cities')}}</h4>
...
@endcan
@canany(['edit_cities','publish_cities','delete_cities'])
...
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/cities/order/{{$row->id}}/{{$i}}"> {{$i}}   </a>
@endfor
</div>
</div>
</td>
...
@can('edit_cities')
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/cities/{{$row->id}}/edit"><i class="mdi mdi-pencil mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.edit')}}</a>
@endcan

{{-- @can('publish_cities')
@if( $row->status == 1 )
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/cities/status/{{$row->id}}/0"><i class="mdi mdi-close mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.block')}}</a>
@else
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/cities/status/{{$row->id}}/1"><i class="mdi mdi-check-all mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.approve')}} </a>
@endif
@endcan --}}

@can('delete_cities')
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/cities/delete/{{$row->id}}"><i class="mdi mdi-delete mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.delete')}}</a>
@endcan
...
<title>{{__('lang.cities')}} | {{config('settings.ProjectName')}}</title>
