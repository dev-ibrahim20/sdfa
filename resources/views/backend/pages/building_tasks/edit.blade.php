...
<h4 class="header-title mb-6">{{__('lang.update_building_tasks')}} | {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/building_tasks/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.building_tasks.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_building_tasks')}} | {{$data->title}} | {{config('settings.ProjectName')}}</title>
