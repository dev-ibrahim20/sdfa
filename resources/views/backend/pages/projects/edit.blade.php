...
<p class="header-title mb-6"> <B> {{__('lang.update_project')}} | {{$project->title}} </B> </p>

{!! Form::model( $project ,[ 'url' =>  config('settings.BackendPath').'/projects/'.$project->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.projects.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_project')}} | {{$project->title}} | {{config('settings.ProjectName')}}</title>
