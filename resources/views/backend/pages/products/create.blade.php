...
<p class="header-title mb-6"> <B> {{__('lang.create_product')}}</B> </p>

{!! Form::open([ 'route' => 'product_store', 'role' => 'form' , 'class' => 'form' , 'files'=>'true' ]) !!}
@include('backend.pages.products.form')
{!!Form::close()!!}
</div>
                            </div><!-- end col -->
                        </div>
...
<title>  {{__('lang.create_product')}} |  {{config('settings.ProjectName')}}</title>
