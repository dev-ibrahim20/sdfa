...
<h4 class="header-title mb-6">{{__('lang.update_samplings')}} | {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/samplings/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
...
<title>{{__('lang.update_samplings')}} | {{$data->title}} | {{config('settings.ProjectName')}}</title>
