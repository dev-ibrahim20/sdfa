...
<h4 class="header-title mb-6">{{__('lang.update_workplaces')}} | {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/workplaces/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.workplaces.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_workplaces')}} | {{$data->title}} | {{config('settings.ProjectName')}}</title>
