...
<p class="header-title mb-6"> <B> {{__('lang.update_contractor')}} | {{$contractor->title}} </B> </p>
{!! Form::model( $contractor ,[ 'url' =>  config('settings.BackendPath').'/contractors/'.$contractor->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.contractors.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_contractor')}} | {{$contractor->title}} | {{config('settings.ProjectName')}}</title>
