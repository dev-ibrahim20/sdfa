...
<p class="header-title mb-6"> <B> {{__('lang.update_category')}} | {{$category->title}} </B> </p>
{!! Form::model( $category ,[ 'url' =>  config('settings.BackendPath').'/categories/'.$category->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
...
<title>{{__('lang.update_category')}} | {{$category->title}} | {{config('settings.ProjectName')}}</title>
