...
<a href="{{url('')}}/{{config('settings.BackendPath')}}/categories/create?category_id={{$category_id ?? ''}}"><button type="button" class="btn btn-danger btn-rounded mb-3"><i class="mdi mdi-plus"></i> {{__('lang.create_category')}} </button></a>
@endcan
...
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/categories/{{$category->id}}/edit"> <i class="fas fa-pencil-alt"> </i> {{__('lang.edit')}}</a>

<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/categories/sub_categories/{{$category->id}}"> <i class="fas fa-pencil-alt"> </i> {{__('lang.sub_categories')}}</a>


<a href="#" class="dropdown-item get_activity_log" subject_id="{{$category->id}}" subject_type="{{get_class($category)}}" data-toggle="modal" data-target="#activityLogModal"> <i class="fa fa-history"> </i> {{__('lang.activity_list')}} </a>

</div>
</div> <!-- end dropdown -->
<!-- Title-->
<h5 class="mt-0"><a href="javascript: void(0);" class="text-dark">  {{$category->title}}</a></h5>
...
<title> {{__('lang.categories')}} | {{config('settings.ProjectName')}}</title>
