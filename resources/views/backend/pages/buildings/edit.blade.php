...
<p class="header-title mb-6"> <B> {{__('lang.update_project')}} | {{$building->title}} </B> </p>

{!! Form::model( $building ,[ 'url' =>  config('settings.BackendPath').'/buildings/'.$building->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.buildings.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_building')}} | {{$building->title}} | {{config('settings.ProjectName')}}</title>
