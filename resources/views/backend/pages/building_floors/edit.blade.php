...
<h4 class="header-title mb-6">{{__('lang.update_building_floors')}} | {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/building_floors/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
@include('backend.pages.building_floors.form')
{!!Form::close()!!}
...
<title>{{__('lang.update_building_floors')}} | {{$data->title}} | {{config('settings.ProjectName')}}</title>
...
var buildings_menu = $('select[name="building_id"]');
