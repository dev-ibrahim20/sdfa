...
<a href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/create" class="btn btn-sm btn-blue waves-effect     waves-light float-right">
<i class="mdi mdi-plus-circle"></i> {{__('lang.add_new')}}
</a>


<h3 class="header-title mb-4"> Building Floors </h4>
...
@canany(['edit_building_floors','publish_building_floors','delete_building_floors'])
...
@foreach ( $data as $row )
<tr >
<td>{{$row->building->project->title ?? ''}}</td>
<td>{{$row->building->title ?? ''}}</td>
<td>{{$row->title}}</td>
...
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/order/{{$row->id}}/{{$i}}"> {{$i}}   </a>
@endfor
</div>
</div>
</td>
...
@can('edit_building_floors')
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/{{$row->id}}/edit"><i class="mdi mdi-pencil mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.edit')}}</a>
@endcan

@can('publish_building_floors')
@if( $row->status == 1 )
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/status/{{$row->id}}/0"><i class="mdi mdi-close mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.block')}}</a>
@else
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/status/{{$row->id}}/1"><i class="mdi mdi-check-all mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.approve')}} </a>
@endif
@endcan

@can('delete_building_floors')
<a class="dropdown-item" href="{{url('')}}/{{config('settings.BackendPath')}}/building_floors/delete/{{$row->id}}"><i class="mdi mdi-delete mr-2 text-muted font-18 vertical-middle"></i>{{__('lang.delete')}}</a>
@endcan
...
<title>{{__('lang.building_floors')}} | {{config('settings.ProjectName')}}</title>
